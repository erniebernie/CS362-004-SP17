...
void cardEffectAdventurer(struct gameState *state, int currentPlayer, int cardDrawn, int *temphand, int z, int drawntreasure){
    ...
    //BUG2 - AS2
    //while(z-1<=0){ //>=0)
    while(z-1>=0){ //AS3 I took the bug out so it can run
	state->discard[currentPlayer][state->discardCount[currentPlayer]++]=temphand[z-1]; // discard all cards in play that have been drawn
	z=z-1;
    }
} //end cardEffectAdventurer
...
void cardEffectSmithy(struct gameState *state, int currentPlayer, int handPos, int choice1){
    ...
    //BUG1 - AS2
    //for (i = 0; i < 3; i--) //++)
    for (i = 0; i < 3; i++) //AS3 I took this bug out so the code with run
    {
	drawCard(currentPlayer, state);
    }
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    switch( card ) 
    {
	//Start of the Refactor code
	case adventurer:
	    cardEffectAdventurer(state,currentPlayer,cardDrawn,temphand,z,drawntreasure);
	    return 0;
	case council_room:
	    cardEffectCouncil_room(state,currentPlayer, handPos);
	    return 0;
	case feast:
	    cardEffectFeast(state, currentPlayer, choice1);
	    return 0;
	case gardens:
	    return -1;
	case mine:
	    return cardEffectMine(state,currentPlayer,handPos, choice1, choice2);
	    return 0;
	case remodel:
	    cardEffectRemodel(state, currentPlayer,handPos, choice1, choice2);
	    return 0;
	case smithy:
	    cardEffectSmithy(state, currentPlayer,handPos, choice1);
	    return 0;
	//End of the Refactor code
